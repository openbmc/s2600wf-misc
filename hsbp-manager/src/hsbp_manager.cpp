...
struct HsbpConfig
{
    ...
    void printConfig()
    {
        ...
        std::cerr << "\trootBus : " << rootBus << "\n";

        std::cerr << "\tsupportedHsbps : ";
        for (auto& hsbp : supportedHsbps)
        {
            std::cerr << hsbp << ", ";
        }
        std::cerr << "\n";

        std::cerr << "\thsbpNvmeMap :\n";
        for (auto& [hsbp, map] : hsbpNvmeMap)
        {
            std::cerr << "\t\t" << hsbp << ": ";
            for (auto& m : map)
            {
        ...
        std::cerr << "\tclockBufferTypes : ";
        for (auto& clkBuffType : clockBufferTypes)
        {
            std::cerr << clkBuffType << ", ";
        }
        std::cerr << "\n";

        std::cerr << "\tioExpanderTypes : ";
...
class ClockBuffer
{
    ...
    void print()
    {
        ...
        std::cerr << "\tmodeOfOperation : " << modeOfOperation << "\n";

        std::cerr << "\tbus : " << bus << "\n";

        std::cerr << "\taddress : " << address << "\n";

        std::cerr << "\toutCtrlBaseAddr : " << outCtrlBaseAddr << "\n";

        std::cerr << "\toutCtrlByteCount : " << outCtrlByteCount << "\n";

        std::cerr << "\tbyteMap :\n";
        for (auto& [byte, map] : byteMap)
        {
            std::cerr << "\t\t" << byte << ": ";
            for (auto& m : map)
            {
  ...
  public:
    ...
    bool enableDisableClock(std::list<std::string>& nvmeDrivesInserted,
                            std::list<std::string>& nvmeDrivesRemoved)
    {
        if (modeOfOperation != "SMBus")
        {
...
class IoExpander
{
    ...
    void print()
    {
        ...
        std::cerr << "\tconfIORegAddr : " << confIORegAddr << "\n";
  ...
  public:
    ...
    bool enableDisableOuput(std::list<std::string>& nvmeDrivesInserted,
                            std::list<std::string>& nvmeDrivesRemoved)
    {
        if (nvmeDrivesInserted.empty() && nvmeDrivesRemoved.empty())
        {
...
std::list<ClockBuffer> clockBuffers;
std::list<IoExpander> ioExpanders;
...
struct Backplane : std::enable_shared_from_this<Backplane>
{
    ...
    bool getInsertedAndRemovedNvmeDrives(
        std::list<std::string>& nvmeDrivesInserted,
        std::list<std::string>& nvmeDrivesRemoved)
    {
        ...
        for (uint8_t index = 0; index < 8; index++)
        {
            ...
            {
                nvmeDrivesInserted.emplace_back(nvmeMap->second.at(index));
                std::cerr << name << " : " << nvmeDrivesInserted.back()
                          << " Inserted !\n";
            }
            ...
            {
                nvmeDrivesRemoved.emplace_back(nvmeMap->second.at(index));
                std::cerr << name << " : " << nvmeDrivesRemoved.back()
                          << " Removed !\n";
            }
        }
...
{
    std::cerr << "stopHsbpManager(): Stopping hsbp-manager\n";
...
/****************************************************************************/
void updateHsbpClocks(std::list<std::string>& nvmeDrivesInserted,
                      std::list<std::string>& nvmeDrivesRemoved)
{
    ...
    for (auto& clockBuffer : clockBuffers)
    {
        ...
        {
            /* There is an issue with setting up with this device. Stop the app
             */
    ...
    if (!nvmeDrivesInserted.empty() || !nvmeDrivesRemoved.empty())
    {
        for (auto& ioExpander : ioExpanders)
        {
            ...
            {
                /* There is an issue with setting up with this device. Stop the
                 * app */
                std::cerr << "Critical Error: Initializing \""
...
void scanHsbpDrivesStatus()
{
    ...
    if (appState < AppState::backplanesLoaded)
    {
        std::cerr << "HSBP not initialized ! Cancelling scan of HSBP drives "
                     "status ! \n";
        return;
    }
    ...
    if (hsbpDriveScanInProgress)
    {
        ...
        std::cerr << "HSBP Drives Scan is already in progress";
    ...
    std::cerr << "Scanning for HSBP drives ...\n";
...
void updateAssets()
{
    ...
    conn->async_method_call(
        [drivesLoadedCallback](const boost::system::error_code ec,
            ...
            if (ec)
            {
                std::cerr << "updateAssets(): Error contacting mapper "
                          << ec.message() << "\n";
            ...
            for (const auto& [path, objDict] : subtree)
            {
                ...
                conn->async_method_call(
                    [path, drivesLoadedCallback](
                        ...
                        if (ec2)
                        {
                            std::cerr << "updateAssets(): Error Getting Config "
                                      << ec2.message() << " "
                                      << "\n";
                            drivesLoadedCallback->setError();
                            return;
                        ...
                        {
                            std::cerr << "updateAssets(): Illegal interface at "
                                      << path << "\n";
                            drivesLoadedCallback->setError();
                            return;
                        }
                        ...
                        {
                            std::cerr
                                << "updateAssets(): Illegal drive index at "
                                << path << " " << driveIndex << "\n";
                            drivesLoadedCallback->setError();
                            return;
                        }
                   ...
                   std::string& rootPath)
{
    ...
    conn->async_method_call(
        [muxes](const boost::system::error_code ec,
            ...
            if (ec)
            {
                std::cerr << "populateMuxes() Error contacting mapper "
                          << ec.message() << "\n";
                return;
            }
            ...
            for (const auto& [path, objDict] : subtree)
            {
                ...
                {
                    std::cerr << "populateMuxes(): Cannot get mux type\n";
                    continue;
                }
                ...
                conn->async_method_call(
                    [path, muxes, index](
                        ...
                        {
                            std::cerr
                                << "populateMuxes(): Error Getting Config "
                                << ec2.message() << "\n";
                            return;
                        }
                        auto findBus = values.find("Bus");
                        ...
                        {
                            std::cerr
                                << "populateMuxes(): Illegal configuration at "
                                << path << "\n";
                            return;
                        }
    ...
    const std::shared_ptr<AsyncCallbackHandler>& backplanesLoadedCallback)
{
    ...
    conn->async_method_call(
        [backplanesLoadedCallback](const boost::system::error_code ec,
            ...
            {
                std::cerr
                    << "populateHsbpBackplanes(): Error contacting mapper "
                    << ec.message() << "\n";
            ...
            if (subtree.empty())
            {
                ...
                std::cerr
                    << "populateHsbpBackplanes(): No HSBPs Detected....\n";
                return;
            }
            ...
            for (const auto& [path, objDict] : subtree)
            {
                if (objDict.empty())
                {
                    std::cerr << "populateHsbpBackplanes(): Subtree data "
                                 "corrupted !\n";
                    backplanesLoadedCallback->setError();
                    return;
                }
                ...
                conn->async_method_call(
                    [backplanesLoadedCallback, path,
                     owner](const boost::system::error_code ec2,
                        ...
                        if (ec2)
                        {
                            std::cerr << "populateHsbpBackplanes(): Error "
                                         "Getting Config "
                                      << ec2.message() << "\n";
                            backplanesLoadedCallback->setError();
                            return;
...
void setUpBackplanesAndDrives()
{
     ...
     * updateAssets() and scanHsbpDrivesStatus() or handle error scnenario */
...
void setupBackplanesAndDrivesMatch()
{
    static auto backplaneMatch = std::make_unique<sdbusplus::bus::match_t>(
        *conn,
        ...
        [](sdbusplus::message_t&) {
             ...
             * addition/removal of HSBP backplane. At this stage, all the HSBP's
             * need to be populated again and also assets have to be
             * re-discovered. So, setting state to componentsLoaded and calling
             * setUpBackplanesAndDrives() only if configuration and components
             * loading was completed */
            if (appState <= AppState::componentsLoaded)
            {
                /* Configuration is not loaded yet. Backplanes will be loaded
                 * once configuration and components are loaded. Checking for
                 * equality here as components loaded state will call populate
                 * either ways */
                std::cerr << "setupBackplanesAndDrivesMatch(): Discarding "
                             "Backplane match\n";
                return;
            }
    ...
    static auto drivesMatch = std::make_unique<sdbusplus::bus::match_t>(
        *conn,
        ...
        [](sdbusplus::message_t&) {
            /* This match will be triggered when ever there is a
             * addition/removal of drives. At this stage only assets have to be
             * re-discovered. So, setting state to backplanesLoaded and calling
             * updateAssets() only if all previous states are completed */
            if (appState <= AppState::backplanesLoaded)
            {
                /* Configuration is not loaded yet. Drives will be loaded once
                 * configuration, components and backplanes are loaded. Checking
                 * for equality here as backplanes loaded state will call
                 * updateAssets either ways */
                std::cerr << "setupBackplanesAndDrivesMatch(): Discarding "
                             "Backplane match\n";
                return;
            }
            ...
            /* We will call the function after a small delay to let all
             * the properties to be intialized */
            auto driveTimer = std::make_shared<boost::asio::steady_timer>(io);
            ...
            driveTimer->async_wait(
                [driveTimer](const boost::system::error_code ec) {
                    ...
                    {
                        std::cerr << "driveTimer: Timer error" << ec.message()
                                  << "\n";
                        return;
                    }
                    updateAssets();
    ...
    const std::shared_ptr<AsyncCallbackHandler>& componentsLoadedCallback)
{
    ...
    conn->async_method_call(
        [componentsLoadedCallback](const boost::system::error_code ec,
            ...
            if (ec)
            {
                std::cerr << "loadIoExpanderInfo(): Error contacting mapper "
                          << ec.message() << "\n";
            ...
            for (auto& [path, objDict] : subtree)
            {
                ...
                {
                    std::cerr
                        << "loadIoExpanderInfo(): Subtree data corrupted !\n";
                    componentsLoadedCallback->setError();
                    return;
                }
                ...
                conn->async_method_call(
                    [componentsLoadedCallback](
                        ...
                        if (er)
                        {
                            std::cerr << "loadIoExpanderInfo(): Error Getting "
                                         "Config "
                                      << er.message() << "\n";
                            componentsLoadedCallback->setError();
                            return;
                         ...
                         * ioExpanders vector */
                        ioExpanders.emplace_back(
                            static_cast<size_t>(*bus),
                            static_cast<size_t>(*address),
                            static_cast<size_t>(*confIORegAddr),
                            static_cast<size_t>(*outCtrlBaseAddr),
                            static_cast<size_t>(*outCtrlByteCount), ioMap,
                            *name, *type);
                    },
    ...
    const std::shared_ptr<AsyncCallbackHandler>& componentsLoadedCallback)
{
    ...
    conn->async_method_call(
        [componentsLoadedCallback](const boost::system::error_code ec,
            ...
            if (ec)
            {
                std::cerr << "loadClockBufferInfo(): Error contacting mapper "
                          << ec.message() << "\n";
                componentsLoadedCallback->setError();
                return;
            }
            ...
            for (auto& [path, objDict] : subtree)
            {
                ...
                {
                    std::cerr
                        << "loadClockBufferInfo(): Subtree data corrupted !\n";
                    componentsLoadedCallback->setError();
                    return;
                }
                ...
                conn->async_method_call(
                    [componentsLoadedCallback](
                        ...
                        if (er)
                        {
                            std::cerr << "loadClockBufferInfo(): Error Getting "
                                         "Config "
                                      << er.message() << "\n";
                            componentsLoadedCallback->setError();
                            return;
                        ...
                        {
                            std::cerr << "loadClockBufferInfo(): Incomplete "
                                         "Clock Buffer definition !! \n";
                            componentsLoadedCallback->setError();
                            return;
                        }
                         ...
                         * clockBuffers vector */
                        clockBuffers.emplace_back(
                            static_cast<size_t>(*bus),
                            static_cast<size_t>(*address), *mode,
                            static_cast<size_t>(*outCtrlBaseAddr),
                            static_cast<size_t>(*outCtrlByteCount), byteMap,
                            *name, *type);
                    },
                    service, path, "org.freedesktop.DBus.Properties", "GetAll",
                    intf);
...
void loadHsbpConfig()
{
    ...
    conn->async_method_call(
        [](const boost::system::error_code ec, const GetSubTreeType& subtree) {
            if (ec)
            {
                std::cerr << "loadHsbpConfig(): Error contacting mapper "
                          << ec.message() << "\n";
            ...
            if (subtree.empty())
            {
                ...
                std::cerr << "loadHsbpConfig(): No configuration detected !!\n";
                return;
            }
            ...
            if (subtree.size() != 1)
            {
                std::cerr << "loadHsbpConfig(): Multiple configurations "
                             "detected !!\n";
                /* Critical Error. Stop Application */
                stopHsbpManager();
                return;
            ...
            {
                /* Critical Error. Stop Application */
                std::cerr << "loadHsbpConfig(): Subtree data corrupted !\n";
                stopHsbpManager();
                return;
            }
            ...
            conn->async_method_call(
                [](const boost::system::error_code er,
                   const boost::container::flat_map<std::string,
                    ...
                    if (er)
                    {
                        std::cerr << "loadHsbpConfig(): Error Getting Config "
                                  << er.message() << "\n";
                        /* Critical Error. Stop Application */
                        stopHsbpManager();
                        return;
                    ...
                    {
                        std::cerr << "loadHsbpConfig(): Incomplete HSBP "
                                     "configuration !! \n";
                        /* Critical Error. Stop Application */
                        stopHsbpManager();
                        return;
                    ...
                    {
                        std::cerr << "loadHsbpConfig(): Incomplete HSBP Map "
                                     "details !! \n";
                        /* Critical Error. Stop Application */
                        stopHsbpManager();
                        return;
                     ...
                     * callback */
                    auto componentsLoadedCallback =
                        std::make_shared<AsyncCallbackHandler>(
                            []() {
                                appState = AppState::componentsLoaded;
                                setUpBackplanesAndDrives();
                            },
                ...
                if (key == "HsbpSupported")
                {
                    ...
                    loadTimer->async_wait(
                        [loadTimer](const boost::system::error_code ec) {
                            ...
                            {
                                std::cerr
                                    << "setupHsbpConfigMatch(): Timer error"
                                    << ec.message() << "\n";
...
static void nvmeLvc3AlertHandler()
{
    ...
    if (appState < AppState::backplanesLoaded)
    {
        std::cerr << "HSBP not initialized ! Dropping the interrupt ! \n";
        return;
    }
    ...
    nvmeLvc3AlertEvent.async_wait(
        ...
        [](const boost::system::error_code ec) {
            if (ec)
            {
                std::cerr << "nvmeLvc3AlertHandler(): nvmealert handler error: "
                          << ec.message() << "\n";
            }
    ...
    boost::asio::posix::stream_descriptor& gpioEventDescriptor)
{
    ...
    if (!gpioLine)
    {
        std::cerr << "hsbpRequestAlertGpioEvents(): Failed to find the " << name
                  << " line\n";
        return false;
    }
    ...
    {
        std::cerr
            << "hsbpRequestAlertGpioEvents(): Failed to request events for "
            << name << "\n";
        return false;
    }
    ...
    if (gpioLineFd < 0)
    {
        std::cerr << "hsbpRequestAlertGpioEvents(): Failed to get " << name
                  << " fd\n";
        return false;
    }
    ...
    gpioEventDescriptor.async_wait(
        ...
        [&name, handler](const boost::system::error_code ec) {
            if (ec)
            {
                std::cerr << "hsbpRequestAlertGpioEvents(): " << name
...
int main()
{
    ...
    if (!hsbpRequestAlertGpioEvents("FM_SMB_BMC_NVME_LVC3_ALERT_N",
                                    ...
                                    nvmeLvc3AlertEvent))
    {
        std::cerr << "main(): error: Unable to monitor events on HSBP "
                     "Alert line\n";
        return -1;
    }
    ...
    io.run();
    std::cerr << "main(): Aborting hsbp-manager !\n";
    return -1;
}
