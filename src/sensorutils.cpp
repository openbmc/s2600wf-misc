...
bool GetSensorAttributes(const double max, const double min, int16_t &mValue,
  ...
  while (((mDouble - floor(mDouble)) / mDouble) > (1.0 / 255)) {
    ...
    // check to see if we reached the limit of where we can adjust back the B
    // value
  ...
  while (bDouble > MAX_INT10 || bDouble < MIN_INT10) {
    if (bExp == MAX_INT4) {
      std::cerr << "bExp Too Big, Max and Min range need to be adjusted\n";
      return false;
    }
    bDouble /= 10;
    bExp += 1;
  ...
  while (((fabs(bDouble) - floor(fabs(bDouble))) / fabs(bDouble)) >
         (1.0 / 255)) {
    if (bExp == MIN_INT4) {
      std::cerr << "bExp Too Small, Max and Min range need to be adjusted\n";
      return false;
    }
    bDouble *= 10;
    bExp -= 1;
